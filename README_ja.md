...
| カテゴリ | 機能名 | 詳細説明 | 関連技術・コンポーネント |
| :--- | :--- | :--- | :--- |
| **プロキシ・決済** | **x402プロキシ層** | 上流リソースへのアクセスの前段に立ち、支払い要求(402)をインターセプトして独自の決済フローを提供します。 | `app/api/proxy`, Hono |
| | **スポンサー決済** | ユーザーの代わりにスポンサーがリソース料金を支払う仕組みを統合します。 | `server/core` |
| | **CDPウォレット** | Coinbase Developer Platformを利用した埋め込みウォレットで、USDCによる直接支払いをサポートします。 | `components/cdp-provider` |
| **アクション** | **アクションプラグイン** | メール登録、アンケート、ソーシャルアクションなど、支払いの代わりとなる多様な「行動」を定義・検証します。 | `server/core/actions` |
| | **行動検証** | ユーザーが行ったアクションが有効かどうかを検証します（例: GitHub API, VLayer）。 | `server/blockchain` |
| **外部連携** | **ChatGPT統合** | OpenAI Apps SDKとMCP (Model Context Protocol) を利用し、ChatGPTのインターフェース内でネイティブに動作します。 | `app/mcp`, OpenAI SDK |
| **ユーザー機能** | **リソース探索** | 利用可能なx402保護リソースを検索・閲覧できるディレクトリ機能です。 | `app/resources` |
| | **ダッシュボード** | スポンサーおよびユーザーが利用状況や設定を管理するための管理画面です。 | `app/(dashboard)` |
